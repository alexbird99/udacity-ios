...
var finishedWord: Bool = true
var gotTickets: Bool = true

if (finishedWord && gotTickets) {
    goToConcert()
}
/*:
### Exercise 12
...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if (bugs >= 10) {
        print("\(bugs) bugs now, need less then 10 bugs")
    } else if (!hasMusic) {
        print("no music, need music")
    } else if (numberOfLevels <= 5) {
        print("\(numberOfLevels) levels now, need more than 5 levels")
    } else {
        release()
    }
}
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    ...
    if (bike && run) {
        ...
        print("\(name) cannot finish bike portion.")
    } else {
